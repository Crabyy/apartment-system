        ...
        <div class="q-mb-md">
          ...
          <div class="q-mt-md">
            ...
            <q-btn
              label="Create"
              color="primary"
              @click.prevent="openAdminRegister"
            />
          </div>
        </div>
      </template>
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost/apartment-system/database/include/admin/adminTable.php"
        );
        ...
        keywords.length > 0
          ? originalRows.value.filter((row) => {
              return columns.some((column) => {
                if (
                  ["surname", "givenname", "middlename"].includes(column.field)
                ) {
                  const fullName =
                    `${row["surname"]}, ${row["givenname"]} ${row["middlename"]}`.toLowerCase();
                  ...
                  return formattedDate
                    .toLowerCase()
                    .includes(keywords.join(" ")); // Join keywords for date search
                } else {
