<template>
  <div
    class="admin-profile-container bg-gray-800 rounded-lg text-white border-1 text-lg"
  >
    ...
    <div class="my-profile">
      ...
      <div
        class="flex items-center justify-between mt-6 pb-4"
        v-if="userType === 'user'"
      >
        <button
          type="submit"
      ...
      <EditProfile
        ...
        @profile-updated="updateProfileData"
      />
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 md:gap-2">
      ...
      <div class="p-2 md:p-2">
        ...
        <div class="flex items-center">
          <label
            for="password"
            class="pr-2 font-medium text-gray-900 dark:text-white"
            >Password:</label
          >
          <button
            id="toggleModalBtn"
            class="text-blue-700 font-mediumunderline focus:outline-none"
            type="button"
            @click="showModal"
          >
            Change Password
        ...
        <p class="text-lg pt-1" v-if="userType === 'user'">
    ...
    <div
      v-if="
        (userData =
          userType === 'user' &&
          userData.acquiredUnits &&
          userData.acquiredUnits.length > 0)
      "
      class="p-4"
    >
      <h2
        class="ml-2 text-2xl font-semibold text-white border-b-2 pb-2 mb-4"
      ></h2>
      <h2 class="ml-[10px] text-[30px] pb-1 text-bold">Acquired Units</h2>
      <ul class="space-y-4">
        <li
          v-for="unit in userData.acquiredUnits"
          :key="unit.id"
          class="bg-white shadow-md rounded-lg p-4 flex flex-col md:flex-row justify-between items-start md:items-center"
        >
          ...
          <div class="text-lg font-medium text-gray-700 mt-2 md:mt-0 md:ml-6">
            ...
            <span class="font-semibold text-green-600"
              >Php {{ unit.unitprice.toLocaleString() }}</span
            >
          </div>
        </li>
      </ul>
          ...
          <form class="space-y-4" action="#" @submit.prevent="changePassword">
            <div class="relative">
              ...
              <i
                ...
                class="fas fa-eye-slash absolute top-3 right-2 text-gray-500 cursor-pointer"
              ></i>
              <i
                v-if="showPassword"
                @click="togglePasswordVisibility"
                class="fas fa-eye absolute top-3 right-2 text-gray-500 cursor-pointer"
              ></i>
            </div>
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const updatedUserData = {
            ...this.userData,
            acquiredUnits: response.data.acquiredUnits,
          };
          sessionStorage.setItem("userData", JSON.stringify(updatedUserData));
          this.userData = updatedUserData;
        } else {
          console.error(
            "Failed to fetch acquired units:",
